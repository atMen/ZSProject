...
import customer.tcrj.com.zsproject.adapter.newXslcAdapter;
...
public class NewXCLCFragment extends BaseFragment implements BaseQuickAdapter.OnItemClickListener, newXslcAdapter.OnItemClickListener, newXslcAdapter.OnxgItemClickListener {
    ...
    private List<xslcCxInfo.DataBean> beanList;
    private newXslcAdapter detailAdapter;
    ...
    @SuppressLint("ValidFragment")
    public NewXCLCFragment(String cpmc,String productId, String status) {
...
//        mRecyclerView.setItemAnimator(new SlideInOutRightItemAnimator(mRecyclerView));
        ...
        mRecyclerView.setAdapter(detailAdapter = new newXslcAdapter(beanList, mContext,mRecyclerView,status));
...
//                                loadMoreData(response,false);
                                ...
                                List<xslcCxInfo.DataBean> data = response.getData();
                                loadData(data,false);
                            }
...
//    }
    ...
    //下拉刷新
    private void loadData(List<xslcCxInfo.DataBean> response,boolean isError) {
    ...
    public void onClick(View v) {
        switch (v.getId()){
            case R.id.btn_add:
                ...
                bundle.putString("cpmc",cpmc);

                toClass(mContext,XSLCwriteActivity.class,bundle,REQUESTCODE);
                break;
...
//              toClass(mContext,DialogmsgActivity.class,null,REQUESTCODE);
        ...
        // RESULT_OK，判断另外一个activity已经结束数据输入功能，Standard activity result:
               ...
               if (resultCode == 2) {
                   if (requestCode == REQUESTCODE) {
                       xslcCxInfo.DataBean contentBean = (xslcCxInfo.DataBean) data.getSerializableExtra("three");
                       Log.e("TAG","信息回调");
    ...
    private void updata(xslcCxInfo.DataBean contentBean) {
        detailAdapter.updata(contentBean,num);

    }


    private void add(xslcCxInfo.DataBean s){
    ...
    @Override
    public void OnItemClick(xslcCxInfo.DataBean position, int helperPosition) {
        showUpdateDialog(position,helperPosition);
    }

    private void showUpdateDialog(final xslcCxInfo.DataBean contentBean, final int position) {
...
/*		Intent intent = new Intent();
    ...
    private void sureRemove(xslcCxInfo.DataBean contentBean, final int position) {
        ...
        mMyOkhttp.post()
                .url(ApiConstants.removecpxslistinfoApi)
                .jsonParams(jsonObject.toString())
                .tag(this)
    ...
    private int num;
    @Override
    public void OnXGItemClick(xslcCxInfo.DataBean position, int helperPosition) {
        num = helperPosition;
        Bundle bundle = new Bundle();
